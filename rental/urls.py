...
urlpatterns = [
    ...
    # GET, PUT, PATCH, DELETE methods on owner account. Cannot delete owner account if they own any properties.
    ...
    # GET, PUT, PATCH, DELETE methods on renter account. Cannot delete renter account if they have any properties
    #  reserved.
    url(r'^renters/(?P<pk>[0-9]+)$', views.RenterDetail.as_view(), name='renter-detail'),

    # Create properties for a owner. Assuming only owners can add properties. POST and GET of all properties for a
    #  owner.
    url(r'^owners/(?P<pk>[0-9]+)/properties$', views.OwnerPropertyList.as_view(), name='ownerproperty'),

    # GET, PUT, PATCH, DELETE methods on properties by a owner. Cannot delete a property if there is an existing
    #  reservation.
    url(r'^owners/(?P<pk>[0-9]+)/properties/(?P<pk>[0-9]+)$', views.OwnerPropertyDetail.as_view(),
        name='ownerproperty-detail'),
    ...
    # API to search all properties and can pass any property related fields as query params. Like find available
    #  properties based on start_date, end_date passed as query params.
